...
import com.christain.appbase.common.HttpUtil;
...
public class UpdateService extends Service {
    ...
    private HttpUtil.DownloadCallBack mDownloadCallBack = new HttpUtil.DownloadCallBack() {

        @Override
        public void onDownloading(int progress) {
            if (progress % AppUpdateManager.sNotificationFrequent == 0 || progress == 1 || progress == 100) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (mIsDownloading) {
            new Toastor(this).showToast(R.string.app_download_downloading);
            return super.onStartCommand(intent, flags, startId);
        }

        mDownloadUrl = intent.getStringExtra(AppUpdateManager.KEY_DOWNLOAD_URL);
        if (TextUtils.isEmpty(AppUpdateManager.sDownloadSDPath)) {
            mDownloadSDPath = getPackageName() + "/download";
        } else {
            mDownloadSDPath = AppUpdateManager.sDownloadSDPath;
        }
        ...
        mNotification.icon = AppUpdateManager.sUpdateIcon != 0 ? AppUpdateManager.sUpdateIcon : R.drawable.ptr_rotate_arrow;
        ...
        if (AppUpdateManager.sUpdateIcon != 0) {
            mNotification.contentView.setImageViewResource(R.id.less_app_update_progress_icon, AppUpdateManager.sUpdateIcon);
        }
        mNotificationManager.cancel(NOTIFICATION_ID);
        mNotificationManager.notify(NOTIFICATION_ID, mNotification);
                ...
                if (mDestDir.exists() || mDestDir.mkdirs()) {
                    ...
                    if (mDestFile.exists() && mDestFile.isFile() && checkApkFile(mDestFile.getPath())) {
                        ...
                        try {
                            ...
                            mIsDownloading = true;
                            HttpUtil.download(mDownloadUrl, mDestFile, false, mDownloadCallBack);
                        } catch (Exception e) {
